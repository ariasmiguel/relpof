...
df$Panel <- "ETF"
df$Panel[which(df$Product=="SPY")] <- "Volatility"
...
df <- df %>% mutate(Date=as.Date(rownames(df))) %>% gather(Product,Value,-Date)
df$Panel <- "ETF"
df$Panel[which(df$Product=="SPY")] <- "Skew Index"
...
```{r mixpc}
EPC <- Quandl("CBOE/EQUITY_PC",type="xts",order="asc",start_date=start_date)
IPC <- Quandl("CBOE/INDEX_PC",type="xts",order="asc",start_date=start_date)
TPC <- Quandl("CBOE/TOTAL_PC",type="xts",order="asc",start_date=start_date)
