...
df <- cbind( data.frame( xts::last(vix, "90 days") ),
             data.frame( xts::last(vxmt,"90 days") ), 
             data.frame( xts::last(vxst,"90 days") )) 
df <- df %>% mutate(Date=as.Date(rownames(df))) %>% gather(Product,Value,-Date)
p <- ggplot(df %>% na.omit,aes(x=Date,y=Value,colour=Product)) +
  geom_line(size=stroke_size) +
  labs(x=NULL,y="Volatility Value") +
...
df <- data.frame( xts::last(skew,"90 days") )

df <- df %>% mutate(Date=as.Date(rownames(df))) %>% gather(Product,Value,-Date)
p <- ggplot(df %>% na.omit,aes(x=Date,y=Value,colour=Product)) +
  geom_line(size=stroke_size) +
  labs(x=NULL,y="Skew Value") +
